...
class ChatsSubscription(private val chatsService: ChatsService, private val pubSub: PubSub) : Subscription {
    ...
    suspend fun messageAdded(@GraphQLContext context: MyGraphQLContext): Flux<Message> {
    ...
    suspend fun chatAdded(@GraphQLContext context: MyGraphQLContext): Flux<Chat> {
        return Flux.from(pubSub.chatAdded.filter { withFilter(context, it.id) })
    }

    fun chatRemoved(@GraphQLContext context: MyGraphQLContext): Flux<String> {
